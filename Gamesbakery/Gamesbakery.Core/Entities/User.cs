...
namespace Gamesbakery.Core.Entities
{
    public class User
    {
        ...
        public User(Guid id, string username, string email, DateTime registrationDate, string country, string password, bool isBlocked, decimal balance)
        {
            ...
            if (string.IsNullOrWhiteSpace(country) || country.Length > 50 || !CountryProvider.IsValidCountry(country))
                throw new ArgumentException("Country must be a valid country name (max 50 characters, no spaces).", nameof(country));
